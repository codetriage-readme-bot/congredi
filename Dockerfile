...
RUN apk add -q --update gcc g++ make libffi-dev openssl-dev #115.9 MB
...
# 269 B
RUN pip install -q -r requirements.txt nose2 setuptools-lint pylint # 123.8 MB
ADD . /code
# 174.1 kB
WORKDIR /code
RUN python setup.py build install 1>/dev/null # 46.37 kB
RUN python setup.py bdist_wheel 1>/dev/null # 9.02 kB
RUN python setup.py test 1>/dev/null # 156 kB
RUN python setup.py lint || echo "Code Quality failing..." # 183.1 kB
EXPOSE 8800
